...
final class LevelRepositoryTests: XCTestCase {
	...
	func test_getDefaultLevels_withValidUrlAndJson_shouldReturnCorrectLevels() {
		...
		XCTAssertEqual(
			levels[0].cellsMatrix,
			expectedMatrix1,
			"Expected level 0 matrix to be \(expectedMatrix1), but got \(levels[0].cellsMatrix)."
		)
		XCTAssertEqual(
			levels[1].cellsMatrix,
			expectedMatrix2,
			"Expected level 1 matrix to be \(expectedMatrix2), but got \(levels[1].cellsMatrix)."
		)
		XCTAssertEqual(levels[0].status, .incompleted, "Expected level 0 status to be incompleted.")
		XCTAssertEqual(levels[1].status, .incompleted, "Expected level 1 status to be incompleted.")
	}
	...
	func test_getLevels_withInvalidUrl_shouldReturnDefaultLevels() {
		...
		XCTAssertEqual(
			levels[0].id,
			0,
			"Expected default level ID to be 0, but got \(levels[0].id)."
		)
		XCTAssertEqual(
			levels[0].cellsMatrix,
			expectedMatrix,
			"Expected default level matrix to be \(expectedMatrix), but got \(levels[0].cellsMatrix)."
		)
		XCTAssertEqual(
			levels[0].status,
			.incompleted,
			"Expected default level status to be incompleted, but got \(levels[0].status)."
		)
	}
	...
	func test_getLevels_withInvalidJson_shouldReturnDefaultLevels() {
		...
		let data = makeData(from: invalidJson)
		let url = createTemporaryFile(with: data)
...
private extension LevelRepositoryTests {
	...
	func makeData(from jsonString: String) -> Data {
		guard let data = jsonString.data(using: .utf8) else {
			XCTFail("Failed to convert JSON string to Data using UTF-8 encoding.")
			return Data()
		}
